...
lotteryProject.prototype = {
    ...
    create: function (i, color, isWin) {
        w=this.mx;
        h=this.my;
        var arc = Math.PI / (12 / 2); //根据奖品个数计算圆周角度
        ...
        ctx.font = '16px Microsoft YaHei'; //font 属性设置或返回画布上文本内容的当前字体属性
            ...
            ctx.arc(w, h,180, angle, angle + arc, false);
            ctx.arc(w, h, 30, angle + arc, angle, true);
            ctx.stroke();
            ctx.fill();
            ctx.save();
    ...
    // 旋转
    whirling: function () {
        this.nowIndex = this.nowIndex % this.userSize;
        var fontIndex = this.nowIndex == 0 ? this.userSize-1 : this.nowIndex - 1;
    ...
    draw: function () {
        ...
        var m = this.userSize, _this = this, k = 0;
        for (var i = 0; i < m; i++) {
            setTimeout(function () {
